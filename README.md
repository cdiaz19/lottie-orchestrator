...
**Status:** **`v2.0.0` — "agents that learn, runs that outlive the process."** V2 closes the two gaps against the mid-2026 state of the art. **Self-learning:** a persistent SQLite memory store; a fail-closed write gateway (`MemoryAgent.apply`) that injection- and secret-screens every learned write, dedups it, stamps provenance, and audit-trails it hash-only; recall that reaches the model as **data, never instructions**; opt-in reflexive write-back; episodic trajectory capture; and skill distillation that produces **parameterized prompt templates, never generated Python**, promoted only by a human who declares the capability. **Long-running harness:** context compaction that pins the recall block, and resumable sessions (`lottie run --session`) whose progress survives a crash. A `--learning-delta` benchmark measures whether any of it actually helps. **All of it is opt-in and OFF by default** — the default-on decision was deliberately declined for lack of real-model evidence (see `CHANGELOG.md`). Earlier: `v1.0.0` hardening (rule-11 capability enforcement, BaseAgent/CLI security gate, per-run token cap + TOCTOU-safe cost reservation, HTTP auth/rate-limit/pagination, HITL `edited_input`, `max_turns` + `_verify`), Phase-4 HTTP transports (MCP stdio, OpenAI-compat, Lottie REST, durable resume, real token streaming), governance (audit + policy + cost + OpenTelemetry), and the agent mesh (`v0.3.0`/`v0.4.0`, optional `[mesh]` extra for parallel fan-out, HITL, and time-travel).
...
| _next_ | V3 — Runtime kernel | extract an execution kernel: middleware chain + event stream, module orchestrator, context compiler, provider router, execution planner, plugin SDK | ◻ |
